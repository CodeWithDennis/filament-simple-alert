...
it('sets only the title', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_only_title', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
it('has only the description', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_only_description', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
it('has title and description', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_and_description', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
it('has title, description, and icon', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_description_icon', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
it('has title, description, icon, and color', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_description_icon_color', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
it('has title and link', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_link', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
it('has title, link, and linkLabel', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_link_linkLabel', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
test('title accepts a closure', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_only_title_closure', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
test('description accepts a closure', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_only_description_closure', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
test('icon accepts a closure', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_description_icon_closure', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
test('color accepts a closure', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_description_icon_color_closure', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
test('link accepts a closure', function (string $method, ?string $value) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_title_link_closure', checkFieldUsing: fn ($field) => $value === $field->{$method}()
        )
        ->assertOk();
})
    ->with([
...
test('color can be set via simple color method', function (string $name, string $color, string $icon) {
    Livewire::test(CreateDummy::class)
        ->assertFormFieldExists('alert_simple_'.$name, checkFieldUsing: fn ($field) => $field->getColor() === $color
        )
        ->assertFormFieldExists('alert_simple_'.$name, checkFieldUsing: fn ($field) => $field->getIcon() === $icon
        )
        ->assertOk();
})
    ->with([
